...
func TestReservation(t *testing.T) {
	...
	if !strings.Contains(userRecorder.Body.String(), "2016-12-22T12:00:00Z") {
		t.Errorf("expected status %s; got %s", "2016-12-22 12:00:00", userRecorder.Body.String())
		if userRecorder.Code != http.StatusOK {
			t.Errorf("expected status %s; got %d", http.StatusOK, userRecorder.Code)
		}
	}
	...
	if !strings.Contains(deleteRecorder.Body.String(), "Success") {
		t.Errorf("delete did not occure got :%s", deleteRecorder.Body.String())
		if deleteRecorder.Code != http.StatusOK {
			t.Errorf("expected status %s; got %d", http.StatusOK, deleteRecorder.Code)
		}
	}
	...
	if strings.Contains(userRecorder.Body.String(), "2016-12-22T12:00:00Z") {
		t.Errorf("was not delete got: %v ", userRecorder.Body.String())
		if userRecorder.Code != http.StatusOK {
			t.Errorf("expected status %s; got %d", http.StatusOK, userRecorder.Code)
		}
	}
